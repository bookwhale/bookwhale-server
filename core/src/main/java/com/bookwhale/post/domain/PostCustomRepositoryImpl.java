...
public class PostCustomRepositoryImpl implements PostCustomRepository {
  ...
  public static BooleanExpression titleLike(String title) {
    return Optional.ofNullable(title).map(keyword -> post.book.bookTitle.like("%" + keyword + "%"))
        .orElse(null);
  }

  public static BooleanExpression authorLike(String author) {
    return Optional.ofNullable(author).map(keyword -> post.book.bookAuthor.like("%" + keyword + "%"))
        .orElse(null);
  }

  public static BooleanExpression publisherLike(String publisher) {
    return Optional.ofNullable(publisher).map(keyword -> post.book.bookPublisher.like("%" + keyword + "%"))
        .orElse(null);
  }
